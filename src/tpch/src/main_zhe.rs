...
async fn benchmark_datafusion(opt: DataFusionBenchmarkOpt) -> Result<Vec<RecordBatch>> {
    ...
    debug!("Query {} avg time: {:.2} ms", opt.query, avg);
...
async fn register_tables(opt: &DataFusionBenchmarkOpt, ctx: &SessionContext) -> Result<()> {
    for table in TPCH_TABLES {
        ...
        if opt.mem_table {
            debug!("Loading table '{}' into memory", table);
            let start = Instant::now();
            let memtable =
...
async fn execute_query(ctx: &SessionContext, sql: &str, debug: bool) -> Result<Vec<RecordBatch>> {
    ...
    let result_fut = Abortable::new(collect(physical_plan.clone(), task_ctx), abort_registration);
    
    let (tx, rx) = oneshot::channel();

    tokio::spawn(async move {
        debug!("^^^^^ one: before sender sleeps ^^^^^");
        sleep(Duration::from_micros(10)).await;
        debug!("^^^^^ two: after sender wakes up from sleep to signal to receiver \
                ^^^^^");
    ...
    tokio::spawn(async move {
        match rx.await {
            Ok(v) => {
                debug!("***** three: receiver got signal {:?} from receiver, \
                        about to abort operation *****", v);
                abort_handle.abort();
                debug!("***** four: receiver has called aborted *****");
            },
            Err(_) => debug!("the sender dropped"),
        }
    });

    debug!("====== before main thread sleeps ======");
    sleep(Duration::from_micros(10)).await;
    debug!("====== after main thread wakes up from sleep and now getting results \
            ======");
    let result = result_fut.await.unwrap()?;
